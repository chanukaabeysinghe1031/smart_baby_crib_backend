...
const ecbWeightCurrentSchema = new mongoose.Schema({
  ...
  sysAvgExpectedWeight: { type: Number },
}, { timestamps: true })
    ...
    if (this.isNew) {
        try {
            ...
            if (this.userFedSex === 'M') {
                historicalData = await ecbWeightHistorical.findOne({ avgMarketAge: this.userFedAge })
                if (historicalData) {
                ...
                historicalData = await ecbWeightHistorical.findOne({ avgMarketAge: this.userFedAge })
                if (historicalData) {
                    this.sysAvgExpectedWeight = historicalData.femaleTop50PercentileWeight
                }
            }
            next()
        } catch (err) {
...
const ecbWeightCurrent = mongoose.model('EcbWeightCurrent', ecbWeightCurrentSchema)
