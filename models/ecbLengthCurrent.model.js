...
const ecbLengthCurrentSchema = new mongoose.Schema({
  ...
  userFedBirthLength: { type: Number, required: true },
...
ecbLengthCurrentSchema.pre('save', async function(next) {
    ...
    if (this.userFedSex === 'M') {
        historicalData = await ecbLengthHistorical.findOne({ avgMarketAge: this.userFedAge })
        if (historicalData) {
...
const ecbLengthCurrent = mongoose.model('EcbLengthCurrent', ecbLengthCurrentSchema)
