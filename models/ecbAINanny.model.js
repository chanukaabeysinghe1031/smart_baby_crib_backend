  ...
  if (this.isNew) {
      try {
        if (!this.userFedQuestion && !this.userFedVQuestion) {
            return next(new Error('One of userFedQuestion or userFedVQuestion should be provided'))
        }

        if (this.userFedQuestion && this.userFedVQuestion) {
            return next(new Error('Only one of userFedQuestion or userFedVQuestion should be provided'))
        }
